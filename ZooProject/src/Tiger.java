...
public class Tiger extends Animal{
    ...
    void eat(Object food) {
        if (!(food instanceof Animal)){
            System.out.println("El tigre  solo puede comer otros Animales");
        }

        Animal prey = (Animal) food;

        if (this.getThreat() > prey.getThreat()) {
            System.out.println("El Pastor Aleman se come a el/la " + prey.getClass().getSimpleName());
            setEnergy(getEnergy() + prey.getEnergy());
            prey.setAlive(false);
        } else {
            System.out.println("el Pastor Aleman no puede comerse a el/la " + prey.getClass().getSimpleName());
        }
    }
    ...
    public void reproduce(List<Animal> cellAnimals) {
        ...
        if (sameSpeciesCount >= 2) {
            if (ThreadLocalRandom.current().nextInt(100) < 30) {
                Tiger puppy = new Tiger(9, 6, "carnivore", 100, true, 3);
